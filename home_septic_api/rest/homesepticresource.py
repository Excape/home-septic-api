...
sewer_response_model = namespace.model(
    "SewerResponse",
    {
        "address": fields.String(readonly=True, description="Requested Address"),
        "zipcode": fields.String(readonly=True, description="Requested Zip code"),
        "sewertype": fields.String(
            readonly=True, enum=["None", "Yes", "Storm", "Municipal", "Septic"]
        ),
    },
)
...
class HomeSepticResource(Resource):
    ...
    def get(self):
        if not auth.is_authorized(request):
            namespace.abort(
                401, "Unauthorized. Please provide an API key in the X-API-KEY header"
            )
            return
